...
Model $\text{ARIMA}(0,1,0)$ is random walk , $\text{AR}(1)$, $\text{ARIMA}(8,1,0)$.

```{r}
fit
abs(polyroot(c(1,fit2$coef)))>1
source("plot.roots.R")
plot.roots(NULL,polyroot(c(1,fit2$coef)), main="ma roots Model A")
fit3
abs(polyroot(c(1,fit3$coef)))>1
source("plot.roots.R")
plot.roots(NULL,polyroot(c(1,fit3$coef)), main="ma roots Model B")
```
...
```{r fig.height = 4, fig.width = 5}
fit <- arima(netflix[1:length(netflix)], order=c(1,0,0), method="ML")
fit
...
```{r fig.height = 4, fig.width = 5}
fit <- arima(netflix[1:length(netflix)], order=c(8,1,0), method="ML")
fit
