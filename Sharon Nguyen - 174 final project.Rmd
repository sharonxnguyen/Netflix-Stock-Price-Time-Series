...
Given this information we have a possible $\text{ARIMA}(p,d,q)$ model of $\text{ARIMA}(8,1,0)$ or $\text{ARIMA}(0,1,0)$.
...
for (i in 1:nrow(df)) {
  try(arima.obj <- arima(netflix, order=c(df$p[i], 1, df$q[i]), method="ML", optim.control= list(maxit = 1000)))
...
These models would be $\text{ARIMA}(0,1,0)$, $\text{ARIMA}(1,1,0)$, and $\text{ARIMA}(0,1,1)$.
For some of the ARIMA models created using `arima()` function we get a warning of "possible convergence problem: optim gave code = 1" and this is because the normal optimization for the MLE has reached the default maximum number of iterations before convergence.

```{r}
# Final model:
ind <- which.min(df$AICc)
fit <- arima(netflix1, 
             order=c(df$p[ind], 1, df$q[ind]),
             method="ML")
```
